...
public class SpaceServiceImpl implements SpaceService {
    ...
    public Map exitspace(String userid, String spaceid){
        ...
        //找出invition表中的被邀请记录，设置状态为为-1
        List<Invition> invitions;
        InvitionExample invitionExample1 = new InvitionExample();
        InvitionExample.Criteria criteria2 = invitionExample1.createCriteria();
        criteria2.andInviteeEqualTo(userid);
        criteria2.andSpaceidEqualTo(spaceid);
        invitions=invitionMapper.selectByExample(invitionExample1);
        ...
        if(invitions.size()!=0) {
            ...
            criteria.andSpaceidEqualTo(spaceid);
             b = invitionMapper.updateByExampleSelective(invition, invitionExample);
        }
        else b=1;
    ...
    public Map joinSpace(String userid, String spaceid){
        ...
        Space space=new Space();
        String str=space1.getUsers();
        String str1 = str.concat(";" +userid);
        space.setUsers(str1);
        SpaceExample spaceExample = new SpaceExample();
        SpaceExample.Criteria criteria1 = spaceExample.createCriteria();
        criteria1.andSpaceidEqualTo(spaceid);
        if(spaceMapper.updateByExampleSelective(space, spaceExample)==1){
            map.put("message",true);
        }
